...
exports.commands = [
	"race", // gives dnd 5e spell info on anything that matches
    "races" // shows data count
];
        ...
        if (results.length != 0 ) {
            ...
            if (results.length > 1) {
                ...
                fs.readFile(filename, 'utf8', function (err, data) {
                    ...
                    var meat = data;
                    if (others.length > 0) {
                        bot.sendMessage(msg.channel, "I found **" + others.length + "** other races matching that term: ```" + others.join (", ") + "```");
                    }
                    setTimeout( function () {
...
exports.races = {
	usage: "",
	description: "Reloads races and shows count.",
	process: function (bot, msg, args) {
        diretoryTreeToObj(dirTree, function (err, res){
            if(err)
                console.error(err);
